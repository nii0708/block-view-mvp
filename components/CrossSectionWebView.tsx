...
export interface CrossSectionWebViewRef {
  triggerScreenshot: () => void;
}

const CrossSectionWebView = forwardRef<
  CrossSectionWebViewRef,
  CrossSectionWebViewProps
>((props, ref) => {
  const {
    startLat,
    startLng,
    ...
    useMemo(() => {
      try {
        ...
        const blocks = blockModelData.map((block) => ({
          ...
          fe_ok: block.fe_ok,
          co_idw: block.co_idw,
          // Yang digunakan untuk display utama
          concentrate: block.ni_ok !== -99 ? block.ni_ok : null,
        }));
...
CrossSectionWebView.displayName = "CrossSectionWebView";
