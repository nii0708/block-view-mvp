...
const CrossSectionWebView = forwardRef<any, CrossSectionWebViewProps>(
  (
    {
      startLat,
      startLng,
    ...
    useImperativeHandle(ref, () => ({
      ...
      triggerZoom: (type: "in" | "out" | "reset") => {
        if (webViewRef.current) {
          webViewRef.current.injectJavaScript(`
          (function() {
            try {
              if (window.currentZoom && window.svg) {
                switch('${type}') {
                  case 'in':
                    window.svg.transition().duration(300).call(window.currentZoom.scaleBy, 1.3);
                    break;
                  case 'out':
                    window.svg.transition().duration(300).call(window.currentZoom.scaleBy, 0.7);
                    break;
                  case 'reset':
                    window.svg.transition().duration(300).call(window.currentZoom.transform, d3.zoomIdentity);
                    break;
                }
              }
            } catch (error) {
              console.error("Error triggering zoom:", error);
            }
          })();
          true;
        `);
      ...
      try {
        ...
        switch (message.type) {
          ...
          case "chartDimensions":
            // Chart dimensions handled internally now
            break;
          case "progressUpdate":
            if (message.message) {
    ...
    return (
      <View style={styles.container}>
        ...
        {/* WebView without ScrollView - D3 handles all panning/zooming */}
        <WebView
          ref={webViewRef}
          source={{
            html: d3Html,
          ...
          cacheEnabled={true}
          scrollEnabled={false} // Disable WebView's own scroll, let D3 handle everything
        />
